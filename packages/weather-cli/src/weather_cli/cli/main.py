...
from weather_cli.application.ranges import VALID_RANGES
from weather_cli.application.service import WeatherService
from weather_cli.application.station_presets import format_station_presets_for_help
from weather_cli.cli.formatting import render_output
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  Forecast ranges:\n"
            "    rest-of-today Forecast from now through local midnight\n"
            "    next-24h     Rolling 24-hour hourly forecast from now\n\n"
            "Output contract:\n"
            "  JSON output includes `location`, `resolved_coordinates`, `range`, `source`,\n"
            "  optional `station`, and `periods[]`.\n"
            "  Forecast rows use `start` / `end` and observation rows use the observation\n"
            "  timestamp for both `start` and `end`.\n\n"
            "Examples:\n"
            "  weather \"Seattle,WA\" --range today\n"
            "  weather \"Seattle,WA\" --range rest-of-today\n"
            "  weather \"Seattle,WA\" --range next-24h\n"
            "  weather \"Seattle,WA\" --range next-24h --format json\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Forecast ranges: next-24h, rest-of-today."
        ),
    )
    parser.add_argument(
        "--format",
