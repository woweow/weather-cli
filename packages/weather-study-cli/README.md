...
- `sync-s3` shells out to `aws s3 sync` with the `dev` profile by default; pass `--profile ""` to use the AWS CLI default credential chain (for example `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`). After download it can validate the raw tree through the same contract loader used for mock data.
