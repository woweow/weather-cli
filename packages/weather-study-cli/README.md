...
- one local S3 sync path that shells out to `aws s3 sync` (default: no `--profile`, so the AWS default credential chain applies; use `--profile dev` when you rely on a named profile)
...
- `sync-s3` shells out to `aws s3 sync` without `--profile` by default (env keys, `[default]` profile, instance role, etc.). Pass `--profile <name>` when you use a shared-credentials profile such as `dev`. After sync it validates the downloaded raw tree through the same contract loader used for mock data.
